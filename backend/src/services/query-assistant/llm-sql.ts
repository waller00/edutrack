...
- "qué profesores faltaron en junio" → "Attendance" JOIN "User" JOIN "OrgRole" (con "OrgRole"."code" = 'TEACHER') JOIN "Event" (aplicando el filtro de ciclo lectivo del contexto sobre "Event"."schoolYearId"), WHERE "Attendance"."status" IN ('ABSENT_NOT_JUSTIFIED', 'ABSENT_JUSTIFIED') AND EXTRACT(MONTH FROM ("Attendance"."time" AT TIME ZONE 'UTC' AT TIME ZONE 'America/Montevideo')) = 6, sin filtrar año si no lo nombró; devolver persona, fecha civil Uruguay y estado.`
