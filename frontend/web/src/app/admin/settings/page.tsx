...
type SettingsSection = 'operational' | 'profiles'
...
export default function AdminSystemSettingsPage() {
  ...
  function selectSection(nextSection: SettingsSection) {
    ...
    const url = nextSection === 'operational' ? '/admin/settings' : `/admin/settings?section=${nextSection}`
                ...
                <section className="card space-y-4">
                  ...
                  <label className="block space-y-1">
                    <span className="text-sm text-gray-700">
                      Puente entre clases (minutos). Menor a este hueco = un solo par entrada/salida para el bloque;
                      mayor o igual = nueva entrada obligatoria.
                    </span>
                    <input
                      type="number"
                      min={15}
                  ...
                  <p className="text-sm text-gray-600">
                    La política del producto es exigir prueba de vida en altas cuando el proceso no está en modo sólo
                    desarrollo: necesitás <span className="font-mono text-xs">DIDIT_API_KEY</span> y{' '}
                    <span className="font-mono text-xs">DIDIT_WORKFLOW_ID</span> en el backend. Opcionalmente, en desarrollo
                    sin Didit podés usar <span className="font-mono text-xs">ALLOW_REGISTER_WITHOUT_DIDIT=true</span>{' '}
